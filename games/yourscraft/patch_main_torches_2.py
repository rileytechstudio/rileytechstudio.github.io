...
code = code.replace("const sunLight = new THREE.DirectionalLight(0xfffbe8, 0.95);", init_lights + "\nconst sunLight = new THREE.DirectionalLight(0xfffbe8, 0.95);")
...
animate_torch = """
    ...
    if (torchUpdateTick >= 30) {
        ...
        world.chunks.forEach(c => {
            if (c.userData && c.userData.mesh && c.userData.mesh.userData && c.userData.mesh.userData.torches) {
